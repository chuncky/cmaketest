  ...
  for(i = 0; i < 5; i++)
	{
	    rd->Code = (rd->Code << 8) | ReadByte;
	     //serial_outnum(rd->Code);//printf("______rd->Code=%lx___\n",rd->Code);
	     //serial_outstr("\n");
	     uart_printf("%08x\r\n",rd->Code);
	}
}
...
unsigned long LZMA_Decompress( void *pDst, unsigned long *pulDstLen, void *pSrc, unsigned long ulSrcLen )
{
  ...
  unsigned int * test = pSrc, temp;
   ...
   uart_printf("LZMA_Decompress start here.........\r\n");
/*
...
*/
  ...
  {
#ifdef LZMA_DEBUG
    uart_printf("LZMA_Decompress() : Input mem len is zero\r\n");
#endif
    return 1;
  }
  ...
  {
#ifdef LZMA_DEBUG
    uart_printf("LZMA_Decompress() : stream version is not supported\r\n");
#endif
    return 1;
  }
  ...
  for (ii = 0; ii < 4; ii++)
  {
    ...
    {
#ifdef LZMA_DEBUG
      uart_printf("LZMA_Decompress() :  too long file\r\n");
#endif
      return 1;
    }
  }
  ...
  {
#ifdef LZMA_DEBUG
    uart_printf("LZMA_Decompress() : Properties error\r\n");
#endif
    return 1;
  }
  for (pb = 0; prop0 >= (9 * 5);
  ...
  {
#ifdef LZMA_DEBUG
    uart_printf("LZMA_Decompress() : outSize >= *pulDstLen\r\n");
#endif
    return 1;
  }

	//uart_printf("using_bsdiff_upgrade malloc\n\r");
    lzmaInternalData =(void *)malloc(lzmaInternalSize+1);

  //if (outStream == 0 || lzmaInternalData == 0)
  if (lzmaInternalData == 0)
  {
#ifdef LZMA_DEBUG
    uart_printf("LZMA_Decompress() : outStream == 0 || lzmaInternalData == 0\r\n");
#endif
    return 1;
  }
  ...
  #ifdef _LZMA_OUT_READ
  {
    ...
    {
		#ifdef LZMA_DEBUG
		uart_printf("LZMA_Decompress() : dictionary == 0\r\n");
		#endif
		res =1;
	    goto done;
    }
    ...
    for (nowPos = 0; nowPos < outSize;)
    {
      ...
      {
        #ifdef LZMA_DEBUG
		uart_printf("LZMA_Decompress() : res != 0\r\n");
		#endif
		res =1;
        goto done;
      }
...
#endif
  //K_free(lzmaInternalData);

  *pulDstLen = outSize;
#ifdef LZMA_DEBUG
  uart_printf("LZMA_Decompress() : good!\r\n");
#endif
done:

	//uart_printf("using_bsdiff_upgrade free\n\r");
