...
void qspi_enter_mode(uint32_t mode)
{
	uint32_t mcr;
...
void qspi_init_ahb(int lut, int page_size)
{
	...
	buf_cfg = (0x1UL) << 31 | (data_size / 8) << 8;
...
struct qspi_host * qspi_host_init(int cs, int mhz, int use_xip)
{
	...
	if(CHIP_IS_CRANEG)
		host->cs_addr[QSPI_CS_A2] = QSPI0_DB16_FLASH_A2_BASE;
	else
	...
	if(CHIP_IS_CRANEG)
	{
		_writel(QSPI0_DB16_FLASH_A1_TOP & 0xfffffc00, QSPI0_SFA1AD);
	}
	else
	{
