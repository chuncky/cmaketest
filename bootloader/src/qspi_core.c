...
struct spi_flash_chip *chip2=NULL;
...
//	int mhz = 104;
    ...
    if (((chip->mfr_id == SPIFLASH_MFR_XM)||(chip->mfr_id == SPIFLASH_MFR_ZBIT))&&(mhz == 104))
    {
	...
	chip2 = spi_nor_init2(host, cs2, rx_mode, tx_mode, qpi);
	...
	{
		uart_printf("qspi chip 1,size 0x%x\r\n",chip->size);
	}
	
	if (!chip2)
		uart_printf("SPI Nor/NAND flash2 init failed\r\n");
	else
	{
		uart_printf("qspi chip 2,size 0x%x\r\n",chip2->size);
	}
