...
{
	//close keypad interrupt
	kpc_reg->kpc_pc &= ((~KPC_PC_ME) & (~KPC_PC_DE));

	//disable Matrix Keypad and Direct Keypad mode configuration
	kpc_reg->kpc_pc &= ((~KPC_PC_MIE) & (~KPC_PC_DIE));

	//keypad wake event clear
...
BOOL Keypad_OTA_Flag_Check_B(void)
{
	UINT32 key_pressed0;
	...
	volatile int i = 0;
	key_pressed0 = kpc_reg->kpc_as;	
	volatile int polling_num = 0;
#define MAX_POLLING_NUM (1000)		//polling times

	uart_printf("Func:%s,Line:%d\r\n",__func__,__LINE__);
	uart_printf("[%57s]\r\n","---------------Keypad_OTA_Flag_Check_B---------------------");
	ValKeypadRegister(callback);

	//Customer can modify the two array: [row and col] according to their keypad
	...
	//[1] Polling 1000 times, detect if have key pressed down or not
	while(1) {
		if(key_quit || polling_num == MAX_POLLING_NUM)  {
			break;
		}
		Keypad_ISR();
	...
	if(key_quit == 1){	//[2_1] Key has pressed down
		//record the key row and col
		key_pressed0 = kpc_reg->kpc_as;
		...
		//If not update key value, return
		...
		//Note: read register[kpc_as] again!		
		key_pressed0 = kpc_reg->kpc_as;
		...
		if( Keyval_Check(pressed_row_before, pressed_row_after) &&  
			...
			// execute OTA_SD
			DEBUG_INFO("[4_1]Start OTA_sdcard!\r\n");
			return TRUE;
		}else{
			DEBUG_INFO("[4_2]after delaying 0.5s no UPDATE_KEY_FLAG pressed down\r\n");
			return FALSE;
	...
	}else{	//[2_2] No key pressed down after polling 1000 times
		if(polling_num == MAX_POLLING_NUM)
			DEBUG_INFO("[2_2]After polling %d times, not get a key value\r\n",MAX_POLLING_NUM);
		return FALSE;
	}
...
void keypad_test(void)
{
	...
	while(1) {
		...
		DEBUG_INFO("get a key value\r\n");
	}
	DEBUG_INFO("keypad_test end\r\n");
}
