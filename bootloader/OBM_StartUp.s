...
PSR_IR              EQU     0xc0		; the I and R bits in PSR,used to dis/enable IRQ/FIQ
...
restoreContext   
	...
	;;BX LR
        ...
        PRESERVE8
		AREA      IntRam,CODE,READONLY, ALIGN=5 ; /* ALIGN=5 since there is an ALIGN 32 in the code below. This effectively eliminates */
...
;;UINT32 old_level;
;;
;;    old_level =  disableInterrupts();
;;    CPSR = Disable IRQ & FIQ
;;    return(old_level);
;;
...
disableInterrupts
	...
	LDMFD   sp!,    {r1, lr}
 IF :DEF: RTI_VER_2 
	   ;For watchdog debug add by zyzhou
	   IMPORT rti_intoptype_ptr
	   IMPORT rti_intcaller_ptr
	   STMFD SP!,{R0-R1, LR}
	   LDR R0, =rti_intoptype_ptr
	   LDR R0, [R0]
	   MOV R1, #0
	   STR R1, [R0]
	   LDR R0, =rti_intcaller_ptr
	   LDR R0, [R0]
	   MOV R1, LR
	   STR R1, [R0]
	   LDMFD SP!,{R0-R1, LR}
	   ;For watchdog debug end
 ENDIF

	BX      LR

;;******************************************************************************
  EXPORT restoreInterrupts
;; // Restores the I&F bits in the CPSR (Irq & Fiq) to that value of the CPSR passed.
...
restoreInterrupts
	STMFD   sp!,    {r1, lr}
	MRS     R1, CPSR
	AND     R0,R0,#PSR_IR ; clear all its but I and R
	BIC     R1,R1,#PSR_IR
	ORR     R1,R1,R0
 IF :DEF: RTI_VER_2 
		;For watchdog debug add by zyzhou
