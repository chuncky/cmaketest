...
static unsigned int lcd_panel_readid(struct panel_spec *self)
{
	...
	if(NULL == self->panel_if){
		LCDLOGE("ERROR: panel_readid, mcu has not been inited!\r\n");
		return 0;
	}
	...
	if(read_id != 0x85){
		LCDLOGE("ERROR: panel_readid, read 0xDA expect 0x05, but receive 0x%x!\r\n",
			read_id);
		return 0;
	}
	...
	if(read_id != 0x858552){
		LCDLOGE("ERROR: panel_readid, read 0x04 expect 0x858552, but receive 0x%x!\r\n",
			read_id);
		return 0;
	}
