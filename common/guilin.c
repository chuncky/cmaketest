...
#define     GUILIN_RTC_USER_DATA_4_REG          0xEB            
void pm802_userdata_set_app_lcd_ever_configed(void)
{
    unsigned char var = 0;
	
	GuilinRead( GUILIN_BASE_Reg, GUILIN_RTC_USER_DATA_4_REG, &var );
	var |= 0x80; //BIT_7
	GuilinWrite( GUILIN_BASE_Reg, GUILIN_RTC_USER_DATA_4_REG, var );
}

unsigned char pm802_userdata_read_app_lcd_ever_configed(void)
{
    unsigned char var = 0;
	...
	if(var == 0x80)
		return 1;			//panel init success
	else
		return 0;			//panel init fail
}
