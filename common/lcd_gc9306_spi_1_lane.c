...
static unsigned int lcd_panel_readid(struct panel_spec *self)
{
	...
	if (read_id != 0x0){
		LCDLOGE("ERROR: panel_readid, read 0xDA expect 0x0, but receive 0x%x!\r\n",
			read_id);
		return 0;
	}
	...
	if (read_id != 0x93){
		LCDLOGE("ERROR: panel_readid, read 0xDB expect 0x93, but receive 0x%x!\r\n",
			read_id);
		return 0;
	}
	...
	if (read_id != 0x06){
		LCDLOGE("ERROR: panel_readid, read 0xDC expect 0x06, but receive 0x%x!\r\n",
			read_id);
		return 0;
	}
...
static struct spi_info lcd_gc9306_spi_info = {
	3,
	1, 
	1,
	SPI_FORMAT_RGB565,
