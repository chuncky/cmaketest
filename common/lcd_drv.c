...
OSAFlagRef g_lcd_interrupt_flag_ref = NULL;
#endif
...
extern UINT32 INTCGetIntVirtualNum(UINT32 IntAbsNum);
...
static void lcd_isr_handler(uint32_t irq_num)
{
	...
	if(NULL == lcd_ctx){
	//	LCDLOGE("ERROR: lcd_hisr, Invalid param!\r\n");
		return;
	}
	...
	OSAFlagSet(g_lcd_interrupt_flag_ref, flag,OSA_FLAG_OR);
...
static void lcd_start_polling(int wb, int ass_mode)
{
	...
	while (0 == (reg & condition) && timeout >0){
        ...
        mdelay(5);
#endif
		LCDLOGI("Info: lcd_start_polling, wait to finish (0x%x)\r\n",reg);
		reg = lcd_read(IRQ_ISR_RAW);
	}
	lcd_write(IRQ_ISR_RAW, ~reg);
...
#ifdef LCD_INTERRUPT_MODE
static void lcd_start_interrtup(int wb)
{
	int i = 0;
	UINT32 actual_flags = 0;
	UINT32 expect_flag = 0;
	...
	lcd_set_bits(PN_CTRL1, BIT_31);
	LCDLOGI("Info: lcd begin to start: %d!!\r\n",wb);
	#define LCD_WAIT_TIMES 10	
    while(i < LCD_WAIT_TIMES) 
	{
		ret = OSAFlagWait(g_lcd_interrupt_flag_ref, expect_flag, OSA_FLAG_OR_CLEAR, &actual_flags, FRAME_TIMEOUT);		
		if ((ret == OS_SUCCESS) && (0 != (actual_flags & expect_flag)))
		{
			LCDLOGD("DBG: lcd_start_interrtup: got frame or wb done!\r\n");
			break;
		}else
		{
			LCDLOGD("DBG: lcd_start_interrtup:wait to finish (0x%x)(%d)!\r\n", actual_flags, i);
		}
		i++;
    }
...
struct s_lcd_ctx* lcd_init1(int work_mode)
{
	struct s_lcd_ctx *lcd_ctx = &g_lcd_ctx;
	...
	if(work_mode >= LCD_WORK_MODE_LIMIT) {
		LCDLOGE("ERROR: lcd_init1: Invalid work_mode\r\n");
		return NULL;
	}

#ifndef LCD_INTERRUPT_MODE
	if(work_mode == LCD_WORK_MODE_INTERRUPT) {
		LCDLOGW("WARNING: lcd_init1: No interrupt mode supported, change to polling mode!\r\n");
	...
	if(LCD_WORK_MODE_INTERRUPT == lcd_ctx->work_mode){
		osa_status = OSAFlagCreate(&g_lcd_interrupt_flag_ref);
...
void lcd_uninit(struct s_lcd_ctx *lcd_ctx)
{
	...
	{
		lcd_irq_dis();
		OSAFlagDelete(g_lcd_interrupt_flag_ref);
	}
#endif
}
