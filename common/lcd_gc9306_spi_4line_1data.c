...
static unsigned int lcd_panel_readid(struct panel_spec *self)
{
	...
	if(read_id != 0x06){
		LCDLOGD("ERROR: panel_readid, read 0xDC expect 0x06, but receive 0x%x!\r\n",
			read_id);
		return 0;
	}
	...
	if(read_id != 0x9306){
		LCDLOGD("ERROR: panel_readid, read 0x04 expect 0x9306, but receive 0x%x!\r\n",
			read_id);
		return 0;
	}
...
static struct panel_operations lcd_gc9306_spi_ops = {
...
static struct timing_spi lcd_gc9306_spi_timing = {
	52000, /*kHz*/
	2500, /*kHz*/
};

static struct spi_info lcd_gc9306_spi_info_for_4line_1data = {
	4,
	1, 
	1,
	SPI_FORMAT_RGB565,
	...
	SPI_ENDIAN_MSB,
	0,
	&lcd_gc9306_spi_timing,
};

struct panel_spec lcd_gc9306_spi_ver2 = {
	"gc9306_spi",
	0x9306,
	LCD_CAP_NORMAL,
	240,
	...
	LCD_MODE_SPI,
	LCD_POLARITY_POS,
	&lcd_gc9306_spi_info_for_4line_1data,
	NULL,
	&lcd_gc9306_spi_ops,
};
