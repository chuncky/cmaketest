...
#ifdef LCD_PSRAM_TEST
#define TEMP_BUFFER_ADDR 0x7E700000
#else
#define TEMP_BUFFER_ADDR 0xD1200000
#endif
...
int get_format_bpp(uint32_t format)
{
	int bpp = 0;
	switch(format){
	case LCD_FORMAT_RGB565:
...
int get_uv_stride_dividor(uint32_t format)
{
	int dividor = 0;
	switch(format){
	case LCD_FORMAT_RGB565:
	...
	if(layer->format == LCD_FORMAT_RGB888_PACKED){
		...
		if(1 != is_first_layer){
			if(layer->alpha_sel != LCD_ALPHA_LAYER){
				LCDLOGE("Error: get_layer_rgb_data, Invalid alpha setting!\r\n");
				return -1;
			}
		...
		LCDLOGE("Error: get_layer_rgb_data, Invalid layer format (0x%x)!\r\n",
			layer->format);
		return -1;
	}
	*rgb_data = data;
	...
	uint8_t *dest_alpha, int log)
{
	...
	if(NULL == src2){
		*dest_rgb = (*src1)&0xFFFFFF;
		if(dest_format == LCD_FORMAT_RGB888_PACKED)
			...
			LCDLOGE("Error: do_compose, Invalid dest format (0x%x)!\r\n",dest_format);
		}
		return result;
	}
	...
	if(1 == log)
		LCDLOGD("r1 = %d, g1 = %d, b1 = %d, r2 = %d, g2 = %d, b2 = %d\r\n",
	...
	if(alpha_mode == LCD_ALPHA_MODE_NORMAL){
		...
		alpha_temp = alpha1 + alpha2 - alpha1 * alpha2/255;
		if(1 == log)
			LCDLOGD("r3_temp = %d, g3_temp = %d, b3_temp = %d\r\n", r3_temp,
				g3_temp, b3_temp);
	}
	...
	*dest_alpha = (alpha_temp > 255) ? 255 : alpha_temp;
	...
	if(1 == log)
		LCDLOGD("r3 = %d, g3 = %d, b3 = %d, alpha= %d\r\n",(int)r3, (int)g3,
				...
				uint32_t height, uint32_t color, uint32_t align, uint32_t *stride)
{
	...
	LCDLOGD("draw_pure_color: color = 0x%x\r\n", color);

	*stride = ALIGN(width * 4, align);
	porch = *stride - width * 4;
	LCDLOGD("draw_pure_color: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
...
//    uint32_t data[5] = {0x55FF0000,0x5500FF00, 0x550000FF,0x55FFFFFF, 0x55000000};
	...
	*stride = ALIGN(width * 4, align);
	porch = *stride - width * 4;
	LCDLOGD("draw_hcb: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
				...
				uint32_t height, uint32_t cb_num, uint32_t align, uint32_t *stride)
{
    ...
    uint32_t *pdata = (uint32_t*)addr;
    uint32_t data[5] = {0x00FF0000,0x0000FF00, 0x000000FF,0x00FFFFFF, 0x00000000};
//    uint32_t data[5] = {0x80FF0000,0x8000FF00, 0x800000FF,0x80FFFFFF, 0x80000000};
    uint32_t cur_index = 0;
    uint32_t colum_num;
	uint32_t porch = 0;
	uint8_t *pbyte = NULL;
	...
	*stride = ALIGN(width * 4, align);
	porch = *stride - width * 4;
	LCDLOGD("draw_vcb: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
				...
				uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	*stride = ALIGN(width * 4, align);
	porch = *stride - width * 4;
	LCDLOGD("draw_line: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
				...
				uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	*stride = ALIGN(width * 4, align);
	porch = *stride - width * 4;
	LCDLOGD("draw_grey_level: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
					...
					uint32_t width, uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	LCDLOGD("prepare_argb8888_image: stride = 0x%x\r\n", *stride);
}
					...
					uint32_t width, uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	*stride = ALIGN(width * 3, align);
	...
	LCDLOGD("argb8888_to_rgb888: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
					...
					uint32_t width, uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	*stride = ALIGN(width * 4, align);
	porch = *stride - width * 4;
	LCDLOGD("argb8888_to_rgba8888: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
					...
					uint32_t width, uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	*stride = ALIGN(width * 2, align);
	...
	LCDLOGD("argb8888_to_rgb565: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
					...
					uint32_t width, uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	*stride = ALIGN(width * 2, align);
	porch = *stride - width * 2;
	LCDLOGD("argb8888_to_rgb1555: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
					...
					uint32_t width, uint32_t height, uint32_t align, uint32_t *stride)
{
	...
	*stride = ALIGN(width * 4, align);
	porch = *stride - width * 4;
	LCDLOGD("argb8888_to_xyuv444_p1: porch = 0x%x, stride = 0x%x\r\n", porch, *stride);
					...
					uint32_t align, uint32_t *stride_p1, uint32_t *stride_p2)
{
	...
	*stride_p2 = ALIGN(width, align);
	...
	LCDLOGD("argb8888_to_yuv420_p2: p1 = 0x%x, s1 = 0x%x, p2 = 0x%x, s2 = 0x%x\r\n",
					...
					uint32_t *stride_p1, uint32_t *stride_p2, uint32_t *stride_p3)
{
	...
	*dest_addr_p3 = addr_p3;
	LCDLOGD("argb8888_to_yuv420_p3: p1 = 0x%x, s1 = 0x%x, p2 = 0x%x, s2 = 0x%x, p3 = 0x%x, s3 = 0x%x\r\n",
					...
					uint32_t *stride_p1, uint32_t *stride_p2, uint32_t *stride_p3)
{
	...
	*dest_addr_p2 = addr_p2;
	*dest_addr_p3 = addr_p3;
	LCDLOGD("argb8888_to_yuv422_p3: p1 = 0x%x, s1 = 0x%x, p2 = 0x%x, s2 = 0x%x, p3 = 0x%x, s3 = 0x%x\r\n",
		porch_p1, *stride_p1, porch_p2, *stride_p2, porch_p3, *stride_p3);
					...
					uint32_t *stride_p1)
{
	...
	*stride_p1 = ALIGN(width * 2, align);
	...
	LCDLOGD("argb8888_to_yuv422_p1: p1 = 0x%x, s1 = 0x%x", porch_p1, *stride_p1);
					...
					uint32_t *stride_p3)
{
	...
	case LCD_FORMAT_RGB1555:
		...
		break;
	default:
		LCDLOGD("draw_images: Unsupport formate = %d\r\n", format);
	}
	return 0;
}
		...
		uint32_t height, uint32_t stride_src, uint32_t stride_dest,uint32_t format, int log)
{
	...
	if((LCD_FORMAT_RGB565 != format) && (LCD_FORMAT_RGB888_PACKED!= format) &&
		...
		LCDLOGD("compare_buffers: only support RGB565 & RGB888_packet & RGB888A (0x%x)\r\n", format);
		return -1;
	}
		...
		if(res !=0) {
			...
			for(i=0;i<width;i++) {
				...
				if((var1 & 0xFFFFFF)!= (var2 & 0xFFFFFF)) {
					if(1 == log){
						LCDLOGE("mismatch: Ref data is 0x%x; Ref address is 0x%x.\r\n",var1,mem_ptr1);
						LCDLOGE("	  HW data is 0x%x; HW address is 0x%x.\r\n",var2,mem_ptr2);
					}
					res = -1;
				}
				mem_ptr1 += bpp;
			...
			if(res != 0){
				if(log){
					LCDLOGE("Error during check of bytes from address: %d.\r\n",dest + j*stride_dest);
				}
				return -1;
			}
		}
...
int lcd_compare_compose_buffer(struct s_compose_setting *compose_setting, int log)
{
	...
	if(NULL == compose_setting){
		LCDLOGE("Error: lcd_compare_compose_buffer, Invalid paramr\r\n");
		return -1;
	}
	...
	if(NULL == first_layer){
		LCDLOGE("Error: lcd_compare_compose_buffer, No layer is enabled!\r\n");
		return -1;
	}
	...
	for(i = 0; i < compose_setting->height; i++){
		for(j = 0; j < compose_setting->width; j++){
			...
			if(0 != ret){
				LCDLOGE("Error: get_layer_rgb_data fail!\r\n");
				return -1;
			} else {
				if(1 == log)
					LCDLOGI("INFO: get src1 %d, %d rgb data is 0x%x, A is%d\r\n",
						j, i, src1, alpha1);
			}
			...
			if(0 != ret){
				...
				if(1 == log)
					LCDLOGI("INFO: get src2 %d, %d rgb data is 0x%x, A is%d\r\n",
						j, i, src2, alpha2);
			}
			if(has_img){
			...
			if(1 == log)
				LCDLOGI("INFO: compose 0x%x, 0x%x is 0x%x!\r\n", src1, src2, result);
			...
			if(0 != ret){
				...
				if(1 == log)
					LCDLOGI("INFO: get %d, %d rgb data is 0x%x!\r\n", j, i, src2);
			}
			result = do_compose(&src1, &src2, alpha1, alpha2, compose_setting->alpha_mode,
				compose_setting->dest_format, &dest_rgb, &dest_alpha, log);
			if(1 == log)
...
compare:
			if(compose_setting->dest_format == LCD_FORMAT_RGB888_PACKED){
				...
				LCDLOGE("Error: Unsupported dest format (0x%x)!\r\n",
...
#if 0
			if(result != dest_rgb){
				LCDLOGE("Error: Compare fail: (w=%d, h=%d) src = 0x%x, dest = 0x%x\r\n", j, i,
					result, dest_rgb);
				LCDLOGE("Error: dest_addr = 0x%x\r\n",dest_addr);
				ret1 = -1;
				return -1;
			}
#else
			if((dest_r < (src_r - 1)) || (dest_r > (src_r + 1)) ||
				...
				LCDLOGE("Error: Compare fail: (w=%d, h=%d) src = 0x%x, dest = 0x%x\r\n", j, i,
					result, dest_rgb);
				LCDLOGE("Error: dest_addr = 0x%x\r\n",dest_addr);
				LCDLOGE("Error: src_r,g,b,a = %d, %d, %d, %d\r\n",src_r, src_g, src_b, src_a);
				LCDLOGE("Error: dest_r,g,b,a = %d, %d, %d, %d\r\n",dest_r, dest_g, dest_b, dest_a);
				ret1 = -1;
				return -1;
			}
#endif
	...
	LCDLOGI("INFO: compare %d pixel are same\r\n",count);
	return ret1;
}
