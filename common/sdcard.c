...
static UINT32 SdCard_Cmd_Nodata(UINT16 cmd, UINT32 argv, UINT32 base_addr)
{
    ...
    *ptr16 = cmd;
	...
	else if(sdcard_get_mode() == CARD_MODE_POLL)
	{
		...
		{
	    	//SdCard_Wait(2000);
	    	SdCard_Wait(166);	//change by hengshanyang, Owner:yazhouren, 3s --> 1s
			tick_count--;
			if(tick_count == 0 )
			{
				uart_printf("Wait for command %d time out!\r\n",cmd>>8);
...
{
	FRESULT res = f_mount(&fatfs_fs[0], DUMP_VOL, 0);
	extern int sdcard_fatfs_mount_status_g;
	if (res == FR_OK) {
		sdcard_fatfs_mount_status_g = 0;
		uart_printf("SDCARD is unmounted sucessfully\n");
	}
}
