...
void *ta_alloc(size_t num) {
    ...
    if (block != NULL) {
		//uart_printf("%s %d malloc size(%08X) %08X\n\r",__FUNCTION__,__LINE__,num,block->addr);
        return block->addr;
    }
	uart_printf("%s %d malloc size(%08X)failed\n\r",__FUNCTION__,__LINE__,num);
    return NULL;
}
...
int free(void *free)
{
	//uart_printf("%s %d free %08x\n\r",__FUNCTION__,__LINE__,free);
    return ta_free(free);
}
