...
#define MAX_MCU_REF_COUNT 2
#else
#define MAX_MCU_REF_COUNT 1
#endif

struct s_mcu_base_ctx g_mcu_base_ctx;
struct s_mcu_ctx g_mcu_ctx[MAX_MCU_REF_COUNT];
int g_mcu_ref_cnt = 0;
...
{
	LCDLOGD("DBG: mcu_enable+++\r\n");
	lcd_set_bits(SMPN_CTRL, BIT_0);
}

static void mcu_disable(void)
{
	LCDLOGD("DBG: mcu_disable+++\r\n");
...
static void set_mcu_path(struct s_mcu_ctx *mcu_ctx, uint32_t path)
{
	...
	mcu_ctx->base_ctx->cur_path = path;
}
...
static void mcu_config(struct mcu_info *info, uint32_t sclk)
{
...
{
	struct s_mcu_base_ctx *mcu_base_ctx = &g_mcu_base_ctx;
	struct s_mcu_ctx *mcu_ctx = NULL;
	uint32_t reg = 0,scycle,rl,rh,wl,wh;
	int i;

	LCDLOGD("mcu_init (ref_cnt: %d, dev_id:%d)+++\r\n", g_mcu_ref_cnt, info->device_id);
	...
	if(g_mcu_ref_cnt >= MAX_MCU_REF_COUNT){
		LCDLOGE("ERROR: too many mcu devices, max count %d!\r\n", MAX_MCU_REF_COUNT);
		return NULL;
	}

	if(g_mcu_ref_cnt > 0 && mcu_base_ctx->sclk != sclk){
		LCDLOGE("ERROR: The new mcu device's sclk(%d) is not same as the old sclk(%d)!\r\n", sclk, mcu_base_ctx->sclk);
		return NULL;
	}


	if(g_mcu_ref_cnt == 0){
		mcu_base_ctx->base_addr = LCD_BASE_ADDR;
		mcu_base_ctx->sclk = sclk;
		mcu_base_ctx->cur_path = MCU_PATH_IMAGE;
		mcu_base_ctx->cur_cs = 0;
	}

	for(i=0; i<MAX_MCU_REF_COUNT; i++){
		mcu_ctx = &g_mcu_ctx[i];
		if(MCU_STATUS_UNINIT == mcu_ctx->status)
			break;
	}
	if(i == MAX_MCU_REF_COUNT){
		LCDLOGE("ERROR: mcu devices are full!\r\n");
		return NULL;
	}
	g_mcu_ref_cnt++;

	mcu_ctx->base_ctx = mcu_base_ctx;
	...
	mcu_base_ctx->work_mode = work_mode;
...
int mcu_update(struct s_mcu_ctx *mcu_ctx, int32_t work_mode)
{
	...
	mcu_ctx->base_ctx->work_mode = work_mode;
	LCDLOGI("mcu_update ----\r\n");
	return 0;
}
...
int mcu_set_cs(struct s_mcu_ctx *mcu_ctx, uint32_t enable)
{
	...
	mcu_ctx->base_ctx->cur_cs = enable;
	return 0;
}
...
int mcu_write_cmd(struct s_mcu_ctx *mcu_ctx, uint32_t cmd, uint32_t bits)
{
	...
	if((mcu_ctx->info.force_cs == 1) && (mcu_ctx->base_ctx->cur_cs == 0)){
		LCDLOGE("ERROR: mcu_write_cmd, Invalid CS status\r\n");
		return -1;
	}

	mcu_config(&mcu_ctx->info, mcu_ctx->base_ctx->sclk);

	if(mcu_ctx->base_ctx->cur_path == MCU_PATH_IMAGE)
		set_mcu_path(mcu_ctx, MCU_PATH_REGISTER);

	switch(bits){
	case 8:
		reg = (cmd & 0xFF) | BIT_8;
		trigger_mcu(reg, mcu_ctx->base_ctx->work_mode);
		break;
	case 16:
	...
	uint32_t *data,  uint32_t data_bits)
{
	...
	case 24:
		...
		case 8:
			...
			trigger_mcu(reg, mcu_ctx->base_ctx->work_mode);//The first read is dummy
			reg = BIT_9 | BIT_15;
			trigger_mcu(reg, mcu_ctx->base_ctx->work_mode);
...
int mcu_after_refresh(struct s_mcu_ctx *mcu_ctx)
{
	...
	if(mcu_ctx->base_ctx->cur_path != MCU_PATH_REGISTER)
		set_mcu_path(mcu_ctx, MCU_PATH_REGISTER);
	return 0;
}
...
void mcu_uninit(struct s_mcu_ctx *mcu_ctx)
{
	...
	LCDLOGD("mcu_uninit (ref_cnt: %d)+++\r\n", g_mcu_ref_cnt);
	if(mcu_ctx->status != MCU_STATUS_INIT){
		LCDLOGE("ERROR: mcu_uninit, This mcu device has not been initialized!\r\n");
		return;
	}
