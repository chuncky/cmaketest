...
#if (!defined(_ATYPES_H) || !defined(UCHAR))
...
#define TX_THREAD_USER_EXTENSION    /* Define a thread-specific errno for standard "C" or posix support */ \
                                    ULONG    tx_errno;
...
#define set_memory_barrier()		__schedule_barrier();\
									__memory_changed()
...
#define TX_RESTORE                  set_memory_barrier();\
									__asm \
                                    { \
...
#define TX_DISABLE                              interrupt_save =  disableInterrupts();
#define TX_RESTORE                              restoreInterrupts(interrupt_save);
#endif
...
CHAR                            _tx_version_id[] = 
                                    "Copyright (c) 1996-2007 Express Logic Inc. * ThreadX ARM9/GNU Version G5.1.5.0 SN: eval000-115-0501 *";
#else
