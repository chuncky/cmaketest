...
#ifndef __FS_API__H__
#define __FS_API__H__
...
{
    unsigned int  i_no;         // inode number 
    unsigned int  i_size;       // For regular files, the file size in bytes
    unsigned int  i_mode;    // Mode of file 
    unsigned int  i_count;    // has opened count. 
    unsigned int  i_atime;      // Time of last access to the file
    unsigned int  i_mtime;      // Time of last data modification
    unsigned int  i_ctime;      // Time of last status(or inode) change    
}FS_FILE_ATTR;
...
{
	long long iTotalSize;    // Total size
    long long iUsedSize;     // Has used  size 
}FS_INFO;
...
{
	unsigned int	st_mode;		// Mode of file 
	long long	st_size;		// For regular files, the file size in bytes
	unsigned int	st_atime;		// Time of last access to the file
	unsigned int	st_mtime;		// Time of last data modification
	unsigned int	st_ctime;		// Time of last status(or inode) change
	unsigned char	st_name[ FS_FILE_NAME_UNICODE_LEN + LEN_FOR_UNICODE_NULL_CHAR];	// The name of file. 
...
{
    unsigned char  dev_name[16];           // Device name
    FS_DEV_TYPE dev_type;        // Device type.
    BOOL is_root;                        // If is root device.                                   
    unsigned char spare[3];                     // Spare field.
}FS_DEV_INFO;
...
int  FS_ChangeSize(int fd, long long nFileSize);
long long  FS_GetFileSize(int fd);
int  FS_Flush(int fd);
int  FS_GetCurDir(unsigned int uSize,	PSTR pCurDir);
int  FS_ChangeDir(PCSTR pszDirName);
int  FS_ReleaseDev(PCSTR pszDevName);
int  FS_Truncate(int fd, long long nFileSize);
int  FS_GetDeviceInfo(unsigned int* pDeviceCount, FS_DEV_INFO** ppFSDevInfo);
unsigned int  FS_GetVersion(VOID);
int  FS_Open_DIR(PCSTR pszFileName);
int  FS_Close_DIR(FS_HANDLE handle);
int  FS_Get_Next_File(FS_HANDLE handle, DIRENT_INFO * dirinfo);
int  FS_DirCount(const UINT8 * path);
int  FS_DirCountEx(const UINT8 * path, unsigned int* nrFiles, unsigned int* nrSubDirs);
BOOL  FS_IsDirFile(const UINT8 *pszFileName);
int  FS_GetDirSize(const UINT8* dirName, long long * size);
BOOL  FS_IsExist(const UINT8 * FileName);
BOOL  FS_TFCardExist(void);
BOOL  FS_TFCardAvailable(void);
int  FS_SetCurrentDir(const UINT8 * DirName);
int  FS_GetCurrentDir(UINT8 * DirName, UINT32 MaxLength);
int  FS_GetFullPath(UINT8 * buf, int buf_len);
int  FS_Move(const UINT8 * SrcFullPath, const UINT8 * DstFullPath);
int  FS_Copy(const UINT8 * SrcFullPath, const UINT8 * DstFullPath);
int  FS_GetFileInfo(const UINT8 * FileName, _FILE_HAL_STAT *info);
long long  FS_GetFileSizeByName(const UINT8 * FileName);
int  FS_Tell(int iFd);
int  FS_IsEndOfFile(int iFd);
int  FS_Abort(int handle);
int FS_Get_Abort(void);
void Fs_RestoreFactorySettings(void);
...
#define ERR_FS_DIR_LEVEL                                 -4200010
...
// No more file id.
// The function that use it inlude:FS_FindFirstFile();FS_Open().
#define ERR_FS_NO_MORE_FD								-4200017
/******************************************************************************/
...
#define ERR_FS_PATHNAME_TOOLONG                            -4200022
...
//Do this BEFORE calling for "stdio.h"
