...
items used in the Fp Code and validates if they are defined in any
'*.kconfig*' file.
...
-location were config is being defined (there could be multiple declarations)
    ...
    By default it scans from fp base tree
...
fpbase = os.environ.get('FP_BASE')

if fpbase == None:
    print ("env. variable FP_BASE is not set, "
           "ensure you have source fp-env.sh")
    exit(1)
elif not os.path.exists(fpbase):
    print ("env. variable FP_BASE \""+ fpbase +
           "\" does not exist as a valid fp base directory")
    exit(1)

os.environ['srctree'] = os.path.join(fpbase + os.path.sep)
sys.path.append(os.path.join(fpbase,'doc','scripts','genrest'))
...
if args.completelog:
    print('sub dir      = ', os.path.join(fpbase + args.subdir))
    print('complete-log = ', args.completelog)
    print('exclude dirs = ', args.exclude)

conf = kconfiglib.Config(os.path.join(fpbase,'Kconfig'))
search_config_in_file(os.path.join(fpbase + os.path.sep + args.subdir),
