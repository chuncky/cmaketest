...
# Zephyr applications, e.g. fp/samples/hello_world/CMakeLists.txt
# must start with the line:
#
# include($ENV{FP_BASE}/cmake/app/boilerplate.cmake NO_POLICY_SCOPE)
#
...
define_property(GLOBAL PROPERTY FP_LIBS
    BRIEF_DOCS "Global list of all Zephyr CMake libs that should be linked in"
    FULL_DOCS  "Global list of all Zephyr CMake libs that should be linked in.
fp_library() appends libs to this list.")
set_property(GLOBAL PROPERTY FP_LIBS "")

define_property(GLOBAL PROPERTY FP_FULL_PATH_LIBS
    BRIEF_DOCS "Global list of all Zephyr CMake libs that should be linked in"
    FULL_DOCS  "Global list of all Zephyr CMake libs that should be linked in.
fp_library() appends libs to this list.")
set_property(GLOBAL PROPERTY FP_FULL_PATH_LIBS "")


define_property(GLOBAL PROPERTY FP_INTERFACE_LIBS
...
fp_interface_library_named() appends libs to this list.")
set_property(GLOBAL PROPERTY FP_INTERFACE_LIBS "")
...
set(__build_dir ${CMAKE_CURRENT_BINARY_DIR})
...
# It is recommended to always use FP_BASE instead of PROJECT_SOURCE_DIR
# when trying to reference ENV${FP_BASE}.
set(PROJECT_SOURCE_DIR $ENV{FP_BASE})

# Convert path to use the '/' separator
string(REPLACE "\\" "/" PROJECT_SOURCE_DIR ${PROJECT_SOURCE_DIR})

set(FP_BINARY_DIR ${PROJECT_BINARY_DIR})
set(FP_BASE ${PROJECT_SOURCE_DIR})

set(AUTOCONF_H ${__build_dir}/include/generated/autoconf.h)
# Re-configure (Re-execute all CMakeLists.txt code) when autoconf.h changes
set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS ${AUTOCONF_H})

include(${FP_BASE}/scripts/cmake/extensions.cmake)

find_package(PythonInterp 3.4)

include(${FP_BASE}/scripts/cmake/ccache.cmake)
...
add_custom_target(
  distclean
  COMMAND ${CMAKE_COMMAND} -P ${FP_BASE}/scripts/cmake/pristine.cmake
...
if(MSYS)
  execute_process(
    COMMAND
    ${PYTHON_EXECUTABLE} ${FP_BASE}/scripts/check_host_is_ok.py
    RESULT_VARIABLE ret
    )
...
# The CPSDK can be set by 3 sources. Through environment variables,
# through the cmake CLI, and through CMakeLists.txt.
#
# CLI has the highest precedence, then comes environment variables,
# and then finally CMakeLists.txt.
#
# A user can ignore all the precedence rules if he simply always uses
# the same source. E.g. always specifies -DCPSDK= on the command line,
# always has an environment variable set, or always has a set(CPSDK
# cp) line in his CMakeLists.txt and avoids mixing sources.
#
# The selected CPSDK can be accessed through the variable 'CPSDK'.

# Read out the cached board value if present
get_property(cached_cpsdk_value CACHE CPSDK PROPERTY VALUE)

# There are actually 4 sources, the three user input sources, and the
# previously used value (CACHED_CPSDK). The previously used value has
# precedence, and if we detect that the user is trying to change the
# value we give him a warning about needing to clean the build
# directory to be able to change boards.

set(cpsdk_cli_argument ${cached_cpsdk_value}) # Either new or old
if(cpsdk_cli_argument STREQUAL CACHED_CPSDK)
  # We already have a CACHED_CPSDK so there is no new input on the CLI
  unset(cpsdk_cli_argument)
endif()

set(cpsdk_app_cmake_lists ${CPSDK})
if(cached_cpsdk_value STREQUAL CPSDK)
  # The app build scripts did not set a default, The CPSDK we are
  # reading is the cached value from the CLI
  unset(cpsdk_app_cmake_lists)
endif()

if(CACHED_CPSDK)
  # Warn the user if it looks like he is trying to change the CPSDK
  # without cleaning first
  if(cpsdk_cli_argument)
    if(NOT (CACHED_CPSDK STREQUAL cpsdk_cli_argument))
      message(WARNING "The build directory must be cleaned pristinely when changing CPSDK")
    endif()
  endif()

  set(CPSDK ${CACHED_CPSDK})
elseif(cpsdk_cli_argument)
  set(CPSDK ${cpsdk_cli_argument})

elseif(DEFINED ENV{CPSDK})
  set(CPSDK $ENV{CPSDK})

elseif(cpsdk_app_cmake_lists)
  set(CPSDK ${cpsdk_app_cmake_lists})

else()
  message(FATAL_ERROR "CPSDK is not being defined on the CMake command-line in the environment or by the app.")
endif()

assert(CPSDK "CPSDK not set")
message(STATUS "Selected CPSDK ${CPSDK}")

# Use BOARD to search fp/boards/** for a _defconfig file,
# e.g. fp/boards/arm/96b_carbon_nrf51/96b_carbon_nrf51_defconfig. When
# found, use that path to infer the ARCH we are building for.
if(NOT BOARD_ROOT)
  set(BOARD_ROOT ${FP_BASE})
endif()

find_path(BOARD_DIR NAMES "${BOARD}_defconfig" PATHS ${BOARD_ROOT}/scripts/boards/* NO_DEFAULT_PATH)
...
MESSAGE(STATUS "Selected CONF_FILE:${CONF_FILE}")
...
###include(${FP_BASE}/cmake/version.cmake)
###include(${FP_BASE}/cmake/host-tools.cmake)
include(${FP_BASE}/scripts/cmake/kconfig.cmake)
include(${FP_BASE}/scripts/cmake/toolchain.cmake)

###find_package(Git QUIET)
###if(GIT_FOUND)
###  execute_process(COMMAND ${GIT_EXECUTABLE} -C ${FP_BASE} describe
...
###include(${FP_BASE}/scripts/cmake/dts.cmake)
...
###add_subdirectory(${FP_BASE} ${__build_dir})
...
# done after 'add_subdirectory(${FP_BASE} ${__build_dir})'
# because interface libraries are defined while processing that
# subdirectory.
get_property(FP_INTERFACE_LIBS_PROPERTY GLOBAL PROPERTY FP_INTERFACE_LIBS)
foreach(boilerplate_lib ${FP_INTERFACE_LIBS_PROPERTY})
  ...
  #
MESSAGE(STATUS "boilerplate_lib       : ${boilerplate_lib}")
