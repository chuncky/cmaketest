...
typedef struct {
   union  {
      ...
      struct  {
         volatile USB_REGISTER   USB_CMD;                   /* Command register @ BASE = 0x5550_2140 for TPV*/
         volatile USB_REGISTER   USB_STS;                   /* Base + 0x144 Status register */
         volatile USB_REGISTER   USB_INTR;                  /* B + 0x148 Interrupt enable */
         volatile USB_REGISTER   USB_FRINDEX;               /* B + 0x14C Frame index */
         volatile USB_REGISTER   CTRLDSSEGMENT_RESEVERVED;  /* B + 0x150 4G segment selector */
         volatile USB_REGISTER   DEVICE_ADDR;               /* B + 0x154 Device Address */         
         volatile USB_REGISTER   EP_LIST_ADDR;              /* B + 0x158 Endpoint List Address */
         volatile USB_REGISTER   TTCTRL;                    /* B + 0x15C */
         volatile USB_REGISTER   BURSTSIZE;                 /* B + 0x160 */
         volatile USB_REGISTER   TXFILLTUNING;              /* B + 0x164 */
         volatile USB_REGISTER   CI2_RESERVED1;             /* B + 0x168 */
         volatile USB_REGISTER   CI2_RESERVED2;             /* B + 0x16C */
         volatile USB_REGISTER   ULPIVIEWPORT;              /* B + 0x170 */
         volatile USB_REGISTER   CI2_RESERVED3;             /* B + 0x174 */
         volatile USB_REGISTER   EPNAK;                     /* B + 0x178 */
         volatile USB_REGISTER   EPNAKEN;                   /* B + 0x17C */        
         volatile USB_REGISTER   CONFIG_FLAG;               /* B + 0x180 Configured Flag register */
         volatile USB_REGISTER   PORTSCX;                   /* B + 0x184 Port Status/Control just one REG not 8 */
         volatile USB_REGISTER   CI2_RESERVED4[7];          /* B + 0x188 -> x1A3 */
         volatile USB_REGISTER   OTGSC;                     /* B + 0x1A4 */
         volatile USB_REGISTER   USB_MODE;                  /* B + 0x1A8 USB Host/Device mode */
         volatile USB_REGISTER   ENDPT_SETUP_STAT;          /* B + 0x1AC Endpoint Setup Status */
         volatile USB_REGISTER   ENDPTPRIME;                /* B + 0x1B0 Endpoint Initialize */
         volatile USB_REGISTER   ENDPTFLUSH;                /* B + 0x1B4 Endpoint De-initialize */
         volatile USB_REGISTER   ENDPTSTATUS;               /* B + 0x1B8 Endpoint Status */
         volatile USB_REGISTER   ENDPTCOMPLETE;             /* B + 0x1BC Endpoint Interrupt On Complete */
         volatile USB_REGISTER   ENDPTCTRLX[16];            /* B + 0x1C0 -> 0x1FF Endpoint Control, where x = 0.. 15 */
         volatile USB_REGISTER   CI2_DEV_RESERVED5[64];     /* B + 0x200 -> B + 0x2FF */
         volatile USB_REGISTER   MUXCTRL;                   /* B + 0x300 */
         volatile USB_REGISTER   CI2ISR;                    /* B + 0x304 */
         volatile USB_REGISTER   CI2IER;                    /* B + 0x308 */
         volatile USB_REGISTER   CI2OSR;                    /* B + 0x30C */
         volatile USB_REGISTER   CI2_DEV_RESERVED6[3900];   /* B + 0x304 -> 0x3FFF */
